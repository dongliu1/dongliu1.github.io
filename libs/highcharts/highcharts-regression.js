...
(function (H) {
    ...
    H.wrap(H.Chart.prototype, 'init', function (proceed) {
		...
		var rangeMin=(arguments[1].xAxis.min)?(arguments[1].xAxis.min):0;
		var rangeMax=(arguments[1].xAxis.max)?(arguments[1].xAxis.max):0;
        ...
        for (i = 0 ; i < series.length ; i++){
            ...
            if ( s.regression && !s.rendered ) {
                ...
                if (regressionType == "linear") {
                    regression = _linear(sdata,s.regressionSettings.decimalPlaces,rangeMin,rangeMax) ;
                    extraSerie.type = "line";
                }else if (regressionType == "exponential") {
                    regression = _exponential(sdata,rangeMin,rangeMax) 
                }                                
                else if (regressionType == "polynomial"){  
	                var order = s.regressionSettings.order || 2
                    regression = _polynomial(sdata, order, rangeMin,rangeMax) ;                    
                }else if (regressionType == "logarithmic"){
                    regression = _logarithmic(sdata, rangeMin, rangeMax) ;
                }else if (regressionType == "leastsquares"){
                    regression = _leastsquares(sdata, rangeMin, rangeMax) ;
     ...
     * Code extracted from https://github.com/Tom-Alexander/regression-js/
     */
    function _exponential(data, rangeMin, rangeMax) {
		...
		if(!(typeof(rangeMin)=="undefined" && typeof(rangeMax)=="undefined")){
			if(typeof(rangeMin)=="undefined") rangeMin=data[0];
			if(typeof(rangeMax)=="undefined") rangeMax=data[data.length-1];
			for(var d=rangeMin;d<rangeMax;d+=(rangeMax-rangeMin)/100){
				var answer=A * Math.pow(Math.E, B * d);
				results.push([d,answer]);
			}	
		}
     ...
     * 
     */
    function _linear(data, decimalPlaces, rangeMin, rangeMax) {
		...
		if(!(typeof(rangeMin)=="undefined" && typeof(rangeMax)=="undefined")){
			...
			if(typeof(rangeMax)=="undefined") rangeMax=data[data.length-1];
			for(var d=rangeMin;d<rangeMax;d+=(rangeMax-rangeMin)/100){
				var coorY = d * gradient + intercept;
				if (decimalPlaces)
					coorY = parseFloat(coorY.toFixed(decimalPlaces));
				var coordinate = [d, coorY];
				results.push(coordinate);
			}	
		}
     ...
     *  Code extracted from https://github.com/Tom-Alexander/regression-js/
     */
    function _logarithmic(data, rangeMin, rangeMax) {
     ...
     * Code extracted from https://github.com/Tom-Alexander/regression-js/
     */
    function _polynomial(data, order, rangeMin, rangeMax) {
		...
		if(!(typeof(rangeMin)=="undefined" && typeof(rangeMax)=="undefined")){
			...
			for(var d=rangeMin;d<rangeMax;d+=(rangeMax-rangeMin)/100){
				var answer = 0;
				for (var w = 0; w < equation.length; w++) {
					answer += equation[w] * Math.pow(d, w);
				}
				results.push([d, answer]);
			}	
		}
     ...
     * - https://gist.github.com/avibryant/1151823
     */    
	function _leastsquares(data, rangeMin, rangeMax) {
