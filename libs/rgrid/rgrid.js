...
!function($){
    ...
    RGrid.prototype = {
        ...
        , _initEvents: function(){
            ...
            if(opts.checkCol){
                $head.find('th .checkAll').on('click', function(){
                    var _mask=$rgrid.find('.rg-mask').is(":hidden");
                    var _loading=$rgrid.find('.rg-loading').is(":hidden");
                    if(!_mask&&!_loading){
        ...
        , _populate: function(items,callback){
            ...
            if(items && items.length !== 0 && opts.cols){
				...
				/*if(!opts.checkCol&&opts.frozen){
				    var _div="<div class='rg-frozen-column' style='position: absolute;top:0;left:0;bottom: 6px;table-layout:fixed;width:100%;overflow-y: scroll;overflow-x: hidden;'></div>";
				...
				function roxinsert(){
					...
					if(items.length>end){
						...
						that._hideLoading();
						if(!opts.checkCol&&opts.frozen)that._setFrozen();
					}
				}
        ...
        , _insertEmptyRow: function(){
            ...
            /*if(!opts.checkCol&&opts.frozen)$body.append("<div class='rg-frozen-column' style='position: absolute;top:0;left:0;bottom: 0;table-layout: fixed;width:100%;'><tbody></tbody></div>");*/
        }
        ...
        ,_setFrozen:function () {
            ...
            var _top=$head.parent().height();
            var _width=$head.find("th").width();
            var _isUse=false;
            if($rgrid.find(".rg-frozen-column").length){
                $rgrid.find(".rg-frozen-column").remove();
                _isUse=true;
            }
            var _html="<div class='rg-headWrapper rg-frozen-column' style='position:absolute; left:0;top:0;width:"+_width+"px;'>"+$head.parent().html()+"</div>";
            var _head=$(_html).appendTo($rgrid);
                _html="<div class='rg-bodyWrapper rg-frozen-column' style='position:absolute; left:0;top:"+_top+"px;bottom:6px;width:"+_width+"px;'></div>";
            var _body=$(_html).appendTo($rgrid);
            $(_body).append($body.clone());
            $(_head).find("th:not(:eq(0))").remove();
            $("tbody tr",_body).find("td:not(:eq(0))").remove();
            $(_body).children(".rg-body").removeAttr("id").css({"width":_width,"background-color":"#fff"});
            if(_isUse)this._setFrozenScroll($(_body));
        }

        ,_setFrozenScroll:function (_frozenObj) {
            var $body = this.$body;
            var $rgrid = this.$rgrid;
            ...
            //冻结列hover效果
            _frozenObj.find("tr").mouseover(function () {
                var _index=$(this).index();
                $body.find("tr:eq("+_index+")").addClass("rg-bodyHover");
            }).mouseout(function () {
                var _index=$(this).index();
                $body.find("tr:eq("+_index+")").removeClass("rg-bodyHover");
            });
            //table滑动效果
            $body.parent().scroll(function () {
				...
				if(scrollObj=="body"){
					time_last_scroll=(new Date()).valueOf();
                    _frozenObj.scrollTop($(this).scrollTop());
				}
            });
            //table hover效果
            $body.find("tr").mouseover(function () {
                var _index=$(this).index();
                _frozenObj.find("tr:eq("+_index+")").addClass("rg-bodyHover");
            }).mouseout(function () {
                var _index=$(this).index();
                _frozenObj.find("tr:eq("+_index+")").removeClass("rg-bodyHover");
            });
        }
        ...
        , _showLoading: function(stext){
            ...
            $head.find('th .checkAll').attr("readonly",true);
        }
        , _hideLoading: function(){
            ...
            $head.find('th .checkAll').attr("readonly",false);
        }
