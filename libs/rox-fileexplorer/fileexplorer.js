...
(function ($) {																	//闭包限定命名空间
				...
				buttons: {
					" 保 存 ": function() {
						...
						if($.trim($('#dlg_filename').val())==""){
							...
							if(!_fileListAction._save(filename,this))return;
						}
	...
	var _fileListAction={														//文件操作，与服务器交互
		...
		_save:function(filename,dlg){												//保存文件，并执行回调
			var suffix=options.suffix;
			...
			if(iresult=="False"){
                var _html="确认覆盖文件"+filename+"？";
                if(options.saveCoverAlert)_html="存在同名文件:<br/>"+filename+"<br/>是否覆盖？";
                skalert(_html,"提示",function () {
